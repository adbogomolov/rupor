...
class RequestAdmin {
    ...
    const LIMIT_OF_NEW = 25;
    const REQUESTS_ON_PAGE = 10;
    const MINUTES = 60;
    const KILOMMETERS_IN_MINUTE = 1.85;

    public static function status($code) {

        $statuses = array(
            self::SOLVED => 'Решена',
            self::INPROGRESS => 'В процессе рассмотрения',
    ...
    public static function addRequest($data, $authorId = null) {
        ...
        foreach ($request->attributes as $attribute => $value) {
            ...
            if (isset($data->$attribute)) {
                ...
                $request->$attribute = (string) $data->$attribute;
            }
            $request->visible = '1';
            $request->author_id = $authorId;
            $request->status = (string) self::INPROGRESS;
    ...
    public static function addLocation($request, $location) {
        ...
        if (isset($location->address)) {
            ...
            $coordinates['address'] = $coordinates ? $location->address : null;
        }
        if (isset($location->lat) && isset($location->lng) && $location->lat && $location->lng) {

            $coordinates['lat'] = (string) $location->lat;
            $coordinates['lng'] = (string) $location->lng;
            if (!isset($location->address) || !$coordinates['address']) {
                $coordinates['address'] = self::determineAddress($location->lat, $location->lng);
            }
        }
        if (!empty($coordinates) && isset($coordinates['lat'])) {
            $request->lat = (string) $coordinates['lat'];
            $request->lng = (string) $coordinates['lng'];
    ...
    public static function determineAddress($lat, $lng) {
        ...
        $params = 'latlng=' . $lat . ',' . $lng . '&language=ru&sensor=false';
        $data = Yii::app()->CURL->run(self::GEOCODE_URL . $params);
        $json = json_decode($data);
        if (empty($json->results)) {
            return false;
     ...
     * @return array
     */
    public static function addComment($user, $requestId, $message) {
        ...
        if ($comment->validate() && $comment->save()) {
            ...
            $result = (array) $comment->attributes;
            $result['author'] = self::addAuthor($user->id, $user->first_name, $user->last_name);
            $result = array('error' => false, 'comment' => (object) $result);
        } else {
    ...
    public static function addAuthor($userId, $firstName = null, $lastName = null) {
        return $userId ? array(
            'first_name' => $firstName,
            'last_name' => $lastName,
                ) : array(
            'first_name' => UserAdmin::UNREGISTERED_USER,
            'last_name' => '',
        );
    }
    ...
    public static function additionalInfo($requests, $userId = null) {
        ...
        foreach ($requests as $request) {
            ...
            //AUTHOR
            $item['author'] = $request->author_id ? self::addAuthor($request->author_id, $request->author->first_name, $request->author->last_name) :
                    self::addAuthor(null);
            //OFFICERS
    ...
    public static function saveFile($filename, $requestId) {
    ...
    public static function updateFile($filename, $requestId, $angle) {

        $requestFile = RequestFile::model()->find('filename=:filename', array('filename' => $filename));
        if (!$requestFile) {
            return false;
        }
        $requestFile->attributes = array(
    ...
    public static function getInArea($lat, $lng, $radius, $count = null) {
        $options = array(
            'select' => '*, (((acos(sin((' . $lat . '*pi()/180)) * sin((lat*pi()/180))+cos((' . $lat . '*pi()/180)) * cos((lat*pi()/180)) * cos(((' . $lng . ' - lng)*pi()/180))))*180/pi())*'.self::MINUTES.'*'.self::KILOMMETERS_IN_MINUTE.') AS distance',
            'having' => 'distance<:distance',
            'order' => 't.created DESC',
            'params' => array('distance' => $radius)
        );
        if ($count) {
            $options['limit'] = $count;
        }
        $requests = BRRequest::model()->findAll($options);
        return $requests;
    }
