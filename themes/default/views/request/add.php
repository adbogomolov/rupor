<?php
	...
	if($this->layout == '//layouts/iframe')
	{
		...
		/*
		 по идее нужно вообще фильтр ставить на список тем - позволять выбирать только из тем выбранных в виджете
		[17:16:18] Денис Тырсин: да, если тема одна нужно просто скрыть блоки ввода темы и названия, при этом название проблемы будет принимать значение названия темы
		*/
		if(count($tags) == 1 )
		{
			$tmp = $tags;
			$single_tag = array_shift($tmp);
			$single_tag = str_replace('&nbsp;', ' ', $single_tag);
...
<div class="form wrap span7">
	...
	<?php if (Yii::app()->user->isGuest): ?>
		...
		<?php if($iframe)
		{
			...
			<div class="row">
				...
				<?php echo $form->textField($model, 'full_name', array('class' => 'span7')); ?>
				<?php echo $form->error($model, 'full_name'); ?>
			</div>

			<div class="row">
				<?php echo $form->labelEx($model, 'address_or_email'); ?>
				<?php echo $form->textField($model, 'address_or_email', array('class' => 'span7')); ?>
				<?php echo $form->error($model, 'address_or_email'); ?>
			</div>
			<?
		}
		else
	...
	if($single_tag)
	{
		...
		echo CHtml::hiddenField('FormRequest[tags][]', array_shift($ids));
	}
	else
	{
	?>
...
(function(){
	...
	function setPlacemark(center, stop){
		...
		Rupor.geocode.getFullAddress({"center": center}, function(res)
		{
			if (res.main_item)
			{
				if (!stop){
	...
	var active_tags = Rupor.data.get('tags');
	if(active_tags && active_tags.length == 1)
	{
		$('#FormRequest_tags').val(active_tags[0]);
		$('#FormRequest_tags').trigger('liszt:updated');
	}
