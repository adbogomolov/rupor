...
class SiteController extends Controller
{
    ...
    public function actionStats()
    {
		...
		$by_status = Yii::app()->db->createCommand()
			->select("CASE WHEN status=1 THEN 'Решено' WHEN status=2 THEN 'Не решено' ELSE 'В работе' END as status, CASE WHEN status=1 THEN '#83B928' WHEN status=2 THEN '#E62828' ELSE '#F1D728' END as color, COUNT(*) as total")
			->from('request')
			->where('address LIKE :location', array(':location'=>$location.'%'))
			->group('status')
			->queryAll();
		...
		$criteria->select = 't.*, (SELECT COUNT(*) FROM feed WHERE user_id = U.id AND event='.Feed::EVENT_CHANGE_STATUS.' AND data='.Request::STATUS_INPROGRESS.') as count';
                     ...
                     //var $location_itog = '(t.address LIKE :location) and t.email<>\'\'';
                     //echo $location;
                     ...
                     for ($i = 0; $i < sizeof($loc_arr)-1; $i++) {
                        ...
                        if ($location_arr<>'') {
                            $location_arr = $location_arr.' or t.address LIKE \''.$location_arr0.'%\'';
                        } else {
                            $location_arr = $location_arr.' t.address = \''.$location_arr0.'\'';
                        }
                     }
                     //echo $location_arr;
                     
                     //$criteria->condition = '('.$location_arr.') and t.id<>9 and t.email<>\'\'';
                     //$criteria->condition = '(t.address LIKE :location '.$location_arr.') and t.id<>9 and t.email<>\'\'';
                     if ($location_arr == '') {
                        $criteria->condition = 't.id<>9 and t.email<>\'\'';
                     } else {
                        $criteria->condition = '('.$location_arr.') and t.id<>9 and t.email<>\'\'';
                     }
		...
		$criteria = new CDbCriteria;
        $criteria->select = 't.*, (SELECT COUNT(*) FROM request r WHERE r.author_id = t.id and r.address LIKE \''.$location.'%\') as request_count, (SELECT COUNT(*) FROM comment c left join request r on c.request_id = r.id WHERE c.author_id = t.id and r.address LIKE \''.$location.'%\') as comment_count';
                     //$criteria->leftJoin('{{request}} rt', 'rt.author_id = t.id')
                     //$criteria->condition = 'first_name=\'Денис\'';
                     //$criteria->params = array(':location'=>$location.'%');
                     $criteria->condition = 't.id<>1205 and t.officer_id is null';
		$criteria->order = 'request_count DESC';
		$criteria->limit = $limit;
