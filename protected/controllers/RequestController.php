...
class RequestController extends Controller
{
    ...
    public function actionWidget()
    {
		if(!empty($_GET['tags']))
		{
			$data = json_decode(Yii::app()->request->cookies['rupor']->value);
			...
			Yii::app()->request->cookies['rupor'] = new CHttpCookie('rupor', json_encode($data), array('domain' => $_SERVER['SERVER_NAME']));
		}
...
HTML;
	 ...
	 * @return array
	 */
    public function actionOfficers(array $tags=array(), $address=null, array $point=array(), $return = false)
    {
    ...
    public function actionPoints($location)
    {
        ...
        for ($i = 0; $i < sizeof($loc_arr)-1; $i++) {
            $splits = explode(',', $location, -$i);
            $location_arr0 = implode(', ', $splits);
            if ($location_arr<>'') {
//                $location_arr = $location_arr.' or address = \''.$location_arr0.'\'';
//                $rsloc = $location_arr0{sizeof($location_arr0)-1};
                $rsloc = $location{0};
                if ($rsloc <> 'Р') {
                    $location_arr = $location_arr.' or address = \'Россия\'';
                }
            } else {
                $location_arr = $location_arr.'address LIKE \''.$location_arr0.'%\'';
            }
        }
        //echo 'SSUUUSUSUSU:'.$location_arr;
        if ($location_arr == '') {
            $criteria->condition = 'visible';
        } else {
            $criteria->condition = '('.$location_arr.') and visible';
//            $criteria->condition = '('.$location_arr.' or address = \'Россия\') and visible';
        }
     ...
     * @return void
     */
    public function actionGeocode($center, $kind = false, $name = false)
    {
    	$res = Request::buildAddress(array('center' => $center, 'kind' => $kind, 'name' => $name));
    	header('Content-type: application/json;charset=utf-8');
		echo CJSON::encode($res);
    }

    /**
     * Аттач офицеров к запросу, используеться мобильным приложением
     * @param int $id
     */
    public function actionBind_Officers($id)
    {
    	$model = Request::model()->findByPk($id);
    	...
    	{
    		$res->status = 0;
			$res->message = 'Invalid request id';
			echo CJSON::encode($res);
			return;
    	}

    	if (!$model->status == Request::STATUS_SOLVED)
    	{
    		$res->status = 0;
    		$res->message = 'Request already solved';
    		echo CJSON::encode($res);
    		return;
    	}

    	$officersRequests = OfficerRequest::model()->findAll('request_id = :request_id', array(
    			'request_id' => $model->id));

    	if(!empty($officersRequests))
    	{
    		$res->status = 0;
    		$res->message = 'Officers already bind';
    		echo CJSON::encode($res);
    		return;
    	}
    	...
    	$tags = array();
    	foreach($model->tagRequests as $tag)
    	{
    		$tags[] = $tag->tag_id;
    	}

    	$point = array($model->lat, $model->lng);
    	$officers = $this->actionOfficers($tags, $model->address, $point, true);

    	foreach($officers as $officer)
    	{
    		$officerRequest = new OfficerRequest;
    		$officerRequest->tmp_id = 0;
    		$officerRequest->officer_id = $officer['id'];
    		$officerRequest->request_id = $model->id;
    		$officerRequest->save();
    		$res->officers[] = (object)array(
    			'id' => $officer['id'],
    			'link_id' => $officerRequest->id,
    			'post' => $officer['post']
    		);
    	}
