...
class RequestController extends Controller
{
    ...
    public function actionView($id, $from=null)
    {
		...
		// ID tags
		$tags = array();
		foreach($model->tagRequests as $obj)
		{
			$tags[] = $obj->tag_id;
		}
		...
		$officers = array();
		foreach(Officer::model()->byLocation($model->address, array($model->lat, $model->lng))->findAll('email!="" AND global = 0') as $obj)
		{
			$valid = false;
			...
			else
			{
				if (empty($tags) || empty($tags[0]))
					continue;
				foreach($obj->officerTags as $tag)
				{
					if (in_array($tag->tag_id, $tags))
					{
						$valid = true;
			...
			$officer = (object) $obj->attributes;
			$officer->choosen = (bool) isset($choosen_officers[$obj->id]);
		...
		// Для ajax свой файл отображения
    ...
    public function actionPoints($location)
    {
		...
		// Мои проблемы
		if ($request->getParam('my') && !Yii::app()->user->isGuest)
		{
		...
		switch($request->getParam('by'))
		{
			case('feed'):
				...
				$criteria->order = 'lastactive DESC';
