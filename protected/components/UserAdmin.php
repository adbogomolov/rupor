...
class UserAdmin {
    ...
    const TWITTER_URL = 'https://api.twitter.com/1/users/show.json?screen_name=';
    const MAIL_RU_URL = 'http://www.appsmail.ru/platform/api';
    const MAIL_RU_UID_URL = 'http://appsmail.ru/platform/mail/';
    const MAIL_RU_APPI_ID = '702170';
     ...
     * @param string $id
     */
    public static function twInfo($id) {

        $twUrl = self::TWITTER_URL . $id;
        $str = file_get_contents($twUrl);
        $result = json_decode($str);
        $name = array(
     ...
     * @return int
     */
    public static function createBySocialNet($net_type, $id, $userData = null) {
     ...
     * @return boolean
     */
    public static function signInBySocialNet($net_type, $id, $userData) {

        $user = User::model()->findByAttributes(array($net_type => $id));
        $signup = 0;
        if (!$user) {
            $user = self::createBySocialNet($net_type, $id, $userData);
            $signup = 1;
        }
        if ($user) {
    ...
    public static function getFields($fields, $userId) {
        ...
        foreach ($fields as $field) {
            if (in_array($field, $attributes)) {
                ...
                $userData = UserData::model()->findByAttributes(array('user_id' => $userId, 'field' => $field));
    ...
    public static function editUserField($userId, $data) {
        ...
        foreach ($data as $fieldName => $value) {
            ...
            $criteria = new CDbCriteria();
            $criteria->condition = 't.user_id=' . $userId;
            $criteria->join = ' JOIN user_field ON t.field=user_field.id AND user_field.name=\'' . $fieldName . '\'';
...
пароль: ' . $newPassword . '.';
    ...
    public static function uploadProfileImage($userId) {
        ...
        $file->saveAs(Settings::getPath('photo') . $imageName);
        return $imageName;
    }
