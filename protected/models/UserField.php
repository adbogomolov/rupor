...
class UserField extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name, input_type', 'required'),
			array('input_type, required', 'numerical', 'integerOnly'=>true),
			array('name, data', 'length', 'max'=>255),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, name, input_type, required, data', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'userDatas' => array(self::HAS_MANY, 'UserData', 'field_id'),
		);
	}
